...
vec3 CalcPBRLighting(light Light, bool IsDirLight, vec4 base_color, vec3 Normal) {
...
void main() {
    ...
    vec3 TotalLight = CalcPBRLighting(su.lights[0], true, base_color, N);
    for (int i = 1; i < su.nlights; i++) {
        TotalLight += CalcPBRLighting(su.lights[i], false, base_color, N);
    }
