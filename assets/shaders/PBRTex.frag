...
vec3 schlickFresnel(float vDotH, vec4 base_color, float metallic) {
    vec3 F0 = vec3(0.04); // specular color for non-metals

    // use material color for metals
    F0 = mix(F0, vec3(base_color), metallic);
...
vec3 PBRLighting(light Light, vec3 l, vec3 intensity , vec4 base_color, vec3 Normal) {
    ...
    vec3 F = schlickFresnel(vDotH, base_color, metallic);
    ...
    vec3 fLambert = mix(vec3(base_color), vec3(0.0), metallic);
...
void main() {
    ...
    for (int i = 0; i < sunLights; i++) {
		...
		totalLight += PBRLighting(su.lights[i], lightDir, intensity, base_color, N);
	}
    ...
    for (int i = spotIndex; i < spotIndex+spotLights; i++) {
        ...
        totalLight += PBRLighting(lightData, lightDir, intensity, base_color, N);
	}

    // add fixed (indirect) ambient value as a cheap replacement for global illumination.
    float ambientStrength = 0.0015;
    totalLight += (ambientStrength * base_color).xyz;
