...
func (ps *pstag) Load(eng *vu.Engine) error {
	...
	eng.ImportAssets("circle.shd", "line3D.shd", "line2D.shd", "color3D.shd")

	// generate som meshes instead of loading them from disk.
	ps.generateMeshes(eng)
	...
	frame := ps.ui.AddModel("shd:line2D", "msh:frame0")
	frame.SetAt(800, 450, 0).SetScale(900, 500, 0).SetColor(1, 1, 1, 1)

	// draw a 2D circle
	c4 := ps.ui.AddModel("shd:line2D", "msh:circle2D0")
...
// generateMeshes creates mesh assets from data.
// Useful for creating geometric shapes.
func (ps *pstag) generateMeshes(eng *vu.Engine) {

	// 1x1 line box.
	frameMeshData[load.Vertexes] = load.F32Buffer(frameVerts, 2)
	frameMeshData[load.Indexes] = load.U16Buffer(frameIndex)
	eng.MakeMeshes("frame", []load.MeshData{frameMeshData})

	// 2D unit circle by connecting 1000 points.
	...
	eng.MakeMeshes("circle2D", []load.MeshData{circle2DMeshData})

	// 3D unit circle by connecting 1000 points.
	circle3DVerts, circle3DIndex := unitCircle()
	circle3DMeshData[load.Vertexes] = load.F32Buffer(circle3DVerts, 3)
	circle3DMeshData[load.Indexes] = load.U16Buffer(circle3DIndex)
	eng.MakeMeshes("circle3D", []load.MeshData{circle3DMeshData})
}
...
// circle3DMeshData holds points generated by unitCircle.
var circle3DMeshData = make(load.MeshData, load.VertexTypes)
