...
// New initializes the procedural land generator. The seed determines
...
func New(tileSize int, seed int64) Land {
...
func (l *land) Fill(landTile Tile) {
	...
	if len(t.topo) == l.size && len(t.topo[0]) == l.size {
		t.topo.generate(t.zoom, t.ox, t.oy, l.n)
	}
}
...
func (l *land) Fill3D(landTile Tile) {
	t, _ := landTile.(*tile)
	if len(t.topo) == l.size && len(t.topo[0]) == l.size {
		t.topo.generate3D(t.zoom, t.face, t.ox, t.oy, l.n)
	}
}
