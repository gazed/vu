	...
	if file, err = l.getResource(directory, filename); err == nil {
		...
		for _, obj := range objects {
			if faces, derr := l.obj2Data(obj.lines, odata); derr == nil {
				if mesh, merr := l.obj2Mesh(obj.name, odata, faces); merr == nil {
					meshes = append(meshes, mesh)
				} else {
					return meshes, fmt.Errorf("Could not create mesh %s from %s", obj.name, filename)
				}
			} else {
				return meshes, fmt.Errorf("Could not parse %s from %s", obj.name, filename)
			}
		}
	}
	return
...
// obj2Mesh turns the data from .obj format into an internal OpenGL friendly
